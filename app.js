...
function applySettingsToUI() {
    ...
    if (tickerContainer) {
        if (isTickerActive) {
            ...
            tickerContainer.classList.add('hidden'); tickerContainer.classList.remove('flex');
        }
    }

    const titleEl = document.getElementById('dyn-page-title'); if(titleEl) titleEl.innerText = `${siteSettings.brandName} | القائمة الرسمية`;
    const brandEl = document.getElementById('dyn-brand-name'); if(brandEl) brandEl.innerText = siteSettings.brandName;
    const heroTitle = document.getElementById('dyn-hero-title'); if(heroTitle) heroTitle.innerHTML = siteSettings.heroTitle;
    const heroDesc = document.getElementById('dyn-hero-desc'); if(heroDesc) heroDesc.innerText = siteSettings.heroDesc;
    const footerBrand = document.getElementById('dyn-footer-brand'); if(footerBrand) footerBrand.innerText = siteSettings.brandName;
    const footerQuote = document.getElementById('dyn-footer-quote'); if(footerQuote) footerQuote.innerText = siteSettings.footerQuote;
    const footerPhone = document.getElementById('dyn-footer-phone'); if(footerPhone) footerPhone.innerText = siteSettings.footerPhone;
    const footerAddress = document.getElementById('dyn-footer-address'); if(footerAddress) footerAddress.innerHTML = siteSettings.footerAddress;
...
function toggleLiveSearch(show) {
    ...
    if (show) {
        ...
        setTimeout(() => { overlay.classList.add('opacity-100'); input.focus(); }, 10);
        input.value = '';
        results.innerHTML = `<div class="h-full flex flex-col items-center justify-center text-white/50 font-bold mt-10"><i data-lucide="cake" class="w-16 h-16 mb-4 opacity-30"></i><p>ابدأ البحث في قائمة حلويات بوسي...</p></div>`;
        if(window.lucide) lucide.createIcons();
    } else {
...
function performLiveSearch(query) {
    ...
    if (!q) {
        resultsContainer.innerHTML = `<div class="h-full flex flex-col items-center justify-center text-white/50 font-bold mt-10"><i data-lucide="cake" class="w-16 h-16 mb-4 opacity-30"></i><p>ابدأ البحث في قائمة حلويات بوسي...</p></div>`;
    ...
    if (matches.length === 0) {
        resultsContainer.innerHTML = `<div class="flex flex-col items-center justify-center text-white/70 font-bold mt-10 bg-white/5 p-8 rounded-2xl"><i data-lucide="search-x" class="w-12 h-12 mb-4 text-pink-400"></i><p>لم نجد تطابق للبحث عن "${escapeHTML(query)}"</p></div>`;
        if(window.lucide) lucide.createIcons(); return;
    }
...
function closeLightbox() {
    const lb = document.getElementById('gallery-lightbox'); if(!lb) return;
...
function drawProductCard(p, layoutMode = 'grid') {
    ...
    let colSpanClass = ''; if (layoutMode === 'grid' && isFullWidth) { colSpanClass = 'col-span-2 md:col-span-2 lg:col-span-2'; }
    ...
    return `<div id="product-card-${p.id}" class="rounded-[1rem] sm:rounded-2xl shadow-sm hover:shadow-lg border flex flex-col group ${cardPadding} ${colSpanClass} transition-all duration-300 relative h-full" style="background-color: var(--site-bg); border-color: hsl(var(--brand-hue), 80%, 90%);">
        ...
        <div onclick="openProductDetails('${p.id}')" class="${aspectClass} cursor-pointer rounded-lg sm:rounded-xl overflow-hidden border relative shrink-0" style="background-color: hsl(var(--brand-hue), 80%, 97%); border-color: hsl(var(--brand-hue), 80%, 90%);">
        ...
        <div class="mt-2.5 sm:mt-3 flex-1 flex flex-col justify-between">
            <div class="mb-3 cursor-pointer" onclick="openProductDetails('${p.id}')">
                <h4 class="font-bold ${titleClass} leading-tight mb-1 line-clamp-1 hover:text-pink-500 transition-colors" style="color: var(--site-text);">${escapeHTML(p.name)}</h4>
...
function renderCakeBuilder(target) {
    ...
    target.innerHTML = `<div class="rounded-[2.5rem] shadow-xl border overflow-hidden animate-fade-in relative" style="background-color: var(--site-bg); border-color: hsl(var(--brand-hue), 80%, 90%);"><div class="p-6 md:p-10 border-b flex flex-col md:flex-row items-center gap-8" style="background-color: hsl(var(--brand-hue), 80%, 97%); border-color: hsl(var(--brand-hue), 80%, 90%);">${sliderHtml}<div class="flex-1 text-center md:text-right"><h2 class="text-2xl md:text-4xl font-bold mb-4 uppercase tracking-tight" style="color: hsl(var(--brand-hue), 70%, 50%);">تخصيص التورت الملكية 👑</h2><p class="text-sm md:text-base font-bold leading-loose opacity-80" style="color: var(--site-text);">${escapeHTML(descText)}</p></div></div><div class="p-6 md:p-12 space-y-12"><div class="grid grid-cols-1 lg:grid-cols-2 gap-10"><div class="space-y-4"><label class="font-bold text-lg flex items-center gap-2" style="color: var(--site-text);"><i data-lucide="cake" style="color: hsl(var(--brand-hue), 70%, 60%);"></i> نكهة الكيك المفضلة</label><div class="grid grid-cols-2 md:grid-cols-4 gap-3">${flavors.map(fl => `<button onclick="uCake('flv', '${escapeHTML(fl)}')" class="py-3 rounded-xl font-bold border-2 text-sm transition-all ${c.flv === fl ? 'text-white shadow-md scale-105 brand-gradient border-transparent' : 'hover:opacity-80'}" style="${c.flv === fl ? '' : `background-color: var(--site-bg); color: hsl(var(--brand-hue), 70%, 50%); border-color: hsl(var(--brand-hue), 80%, 90%);`}">${escapeHTML(fl)}</button>`).join('')}</div></div><div class="space-y-4"><label class="font-bold text-lg flex items-center gap-2" style="color: var(--site-text);"><i data-lucide="heart" style="color: hsl(var(--brand-hue), 70%, 60%);"></i> عدد الأفراد (رقم زوجي)</label><div class="flex items-center justify-between border rounded-2xl p-2 shadow-inner h-full max-h-[80px]" style="background-color: hsl(var(--brand-hue), 80%, 97%); border-color: hsl(var(--brand-hue), 80%, 90%);"><button onclick="adjP(-2)" class="p-3 rounded-xl border hover:scale-105 transition-all" style="background-color: var(--site-bg); color: hsl(var(--brand-hue), 70%, 60%); border-color: hsl(var(--brand-hue), 80%, 90%);"><i data-lucide="minus" class="w-6 h-6"></i></button><div class="text-center flex flex-col items-center"><span class="text-3xl font-bold" style="color: hsl(var(--brand-hue), 70%, 40%);">${c.ps}</span><span class="text-[9px] font-bold uppercase tracking-widest" style="color: hsl(var(--brand-hue), 70%, 65%);">فرداً</span></div><button onclick="adjP(2)" class="p-3 rounded-xl border hover:scale-105 transition-all" style="background-color: var(--site-bg); color: hsl(var(--brand-hue), 70%, 60%); border-color: hsl(var(--brand-hue), 80%, 90%);"><i data-lucide="plus" class="w-6 h-6"></i></button></div></div></div><div class="p-8 rounded-[2rem] border space-y-8 shadow-inner relative" style="background-color: hsl(var(--brand-hue), 80%, 97%); border-color: hsl(var(--brand-hue), 80%, 90%);"><label class="font-bold text-lg flex items-center gap-2" style="color: var(--site-text);"><i data-lucide="star" style="color: hsl(var(--brand-hue), 70%, 60%);"></i> الشكل الهندسي المختار</label><div class="grid grid-cols-2 md:grid-cols-4 gap-4">${['دائري', 'قلب', 'مربع', 'مستطيل'].map(sh => `<button onclick="setSh('${sh}')" class="py-5 rounded-xl font-bold flex flex-col items-center gap-2 border-2 transition-all ${c.sh === sh ? 'text-white shadow-lg scale-105 brand-gradient border-transparent' : 'hover:opacity-80'}" style="${c.sh === sh ? '' : `background-color: var(--site-bg); color: hsl(var(--brand-hue), 70%, 50%); border-color: hsl(var(--brand-hue), 80%, 90%);`}"><span class="text-sm">${sh}</span>${sh === 'مربع' || sh === 'مستطيل' ? `<span class="text-[9px] opacity-75">${sh === 'مربع' ? `(من ${minSq})` : `(من ${minRect})`}</span>` : ''}</button>`).join('')}</div><label class="flex items-center gap-4 cursor-pointer p-3 rounded-xl border" style="background-color: rgba(var(--site-bg), 0.5);"><input type="checkbox" ${c.trd ? 'checked' : ''} onchange="uCake('trd', this.checked)" class="w-6 h-6 rounded"> <span class="font-bold text-sm md:text-base" style="color: var(--site-text);">هل تفضلون التصميم متعدد الأدوار؟</span></label></div><div class="p-8 rounded-[2rem] border flex flex-col md:flex-row gap-6 shadow-sm" style="background-color: hsl(var(--brand-hue), 30%, 97%); border-color: hsl(var(--brand-hue), 30%, 90%);"><i data-lucide="alert-circle" class="w-10 h-10 flex-shrink-0" style="color: hsl(var(--brand-hue), 60%, 50%);"></i><div class="flex-1"><h4 class="font-bold mb-3 text-lg md:text-xl tracking-tight leading-tight" style="color: hsl(var(--brand-hue), 60%, 40%);">الرعاية الصحية وسلامة الغذاء</h4><p class="text-xs mb-5 font-bold leading-loose opacity-80" style="color: hsl(var(--brand-hue), 60%, 30%);">صحتكم وسلامتكم أولوية؛ نرجو تدوين أي تفاصيل تتعلق بالحساسية الغذائية لضمان تجربة آمنة تماماً.</p><input type="text" value="${escapeHTML(c.alg)}" onchange="uCake('alg', this.value)" placeholder="ملاحظات صحية إن وجدت..." class="w-full p-4 rounded-xl border outline-none focus:ring-2 font-bold text-sm shadow-inner" style="background-color: var(--site-bg); color: var(--site-text); border-color: hsl(var(--brand-hue), 30%, 85%);"></div></div><div class="p-8 rounded-[2rem] border flex flex-col md:flex-row gap-8 shadow-sm relative overflow-hidden" style="background-color: hsl(var(--brand-hue), 10%, 97%); border-color: hsl(var(--brand-hue), 10%, 90%);"><div class="flex-1 space-y-4 relative z-10"><label class="font-bold flex items-center gap-2 text-lg" style="color: var(--site-text);"><i data-lucide="party-popper" class="w-6 h-6" style="color: hsl(var(--brand-hue), 60%, 50%);"></i> طبيعة المناسبة</label><input type="text" value="${escapeHTML(c.occ)}" onchange="uCake('occ', this.value)" placeholder="مثال: عيد ميلاد، زفاف..." class="w-full p-4 rounded-xl border shadow-sm outline-none transition-all font-bold text-sm" style="background-color: var(--site-bg); color: var(--site-text); border-color: hsl(var(--brand-hue), 20%, 85%);"></div><div class="flex-1 space-y-4 relative z-10 border-t md:border-t-0 md:border-r pt-6 md:pt-0 md:pr-8" style="border-color: hsl(var(--brand-hue), 20%, 85%);"><label class="font-bold flex items-center gap-2 text-lg" style="color: var(--site-text);"><i data-lucide="image-plus" class="w-6 h-6" style="color: hsl(var(--brand-hue), 60%, 50%);"></i> تصميم مرجعي</label><div class="relative border-2 border-dashed rounded-xl p-4 text-center hover:opacity-80 transition-all cursor-pointer overflow-hidden group" style="background-color: var(--site-bg); border-color: hsl(var(--brand-hue), 40%, 80%);"><input type="file" accept="image/*" onchange="handleRefImage(event)" class="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10">${c.refImgUrl ? `<div class="relative z-0"><img src="${c.refImgUrl}" class="h-24 w-full object-cover rounded-lg shadow-sm mb-2 border"><span class="text-[10px] font-bold px-3 py-1 rounded-md border inline-block shadow-sm">✓ تم الإرفاق</span></div>` : `<div class="relative z-0 py-2"><div class="w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-2" style="background-color: hsl(var(--brand-hue), 80%, 97%);"><i data-lucide="upload-cloud" class="w-6 h-6"></i></div><span class="text-[10px] font-bold opacity-70 block">اضغط هنا لرفع تصميم أعجبك</span></div>`}</div></div></div><div class="grid grid-cols-1 md:grid-cols-2 gap-10 border-t pt-10" style="border-color: hsl(var(--brand-hue), 80%, 90%);"><div class="space-y-4"><label class="font-bold text-lg flex items-center gap-2" style="color: var(--site-text);"><i data-lucide="camera" style="color: hsl(var(--brand-hue), 70%, 60%);"></i> طباعة صورة</label><div class="space-y-3">${imgOpts.map(it => `<label class="flex items-center justify-between p-4 rounded-xl border-2 cursor-pointer transition-all ${c.img === it.label ? 'shadow-sm scale-[1.01]' : ''}" style="${c.img === it.label ? `background-color: hsl(var(--brand-hue), 80%, 97%); border-color: hsl(var(--brand-hue), 70%, 60%);` : `background-color: var(--site-bg); border-color: hsl(var(--brand-hue), 80%, 95%);`}"><div class="flex items-center gap-3"><input type="radio" name="cakeImg" ${c.img === it.label ? 'checked' : ''} onclick="uCake('img', '${it.label}')" class="w-5 h-5"> <span class="font-bold text-sm" style="color: var(--site-text);">${it.label}</span></div>${it.price > 0 ? `<span class="px-3 py-1.5 rounded-lg border font-bold text-[10px] shadow-sm" style="background-color: var(--site-bg); color: hsl(var(--brand-hue), 70%, 50%); border-color: hsl(var(--brand-hue), 80%, 90%);">+${it.price} ج.م</span>` : ''}</label>`).join('')}</div></div><div class="space-y-6"><div class="space-y-3"><label class="font-bold text-lg flex items-center gap-2" style="color: var(--site-text);"><i data-lucide="pen-tool" style="color: hsl(var(--brand-hue), 70%, 60%);"></i> العبارة المراد تدوينها</label><input type="text" value="${escapeHTML(c.msg)}" onchange="uCake('msg', this.value)" placeholder="يرجى كتابة العبارة بوضوح..." class="w-full p-4 rounded-xl border font-bold text-sm shadow-inner" style="background-color: rgba(var(--site-bg), 0.5); border-color: hsl(var(--brand-hue), 80%, 90%); color: var(--site-text);"></div><div class="flex flex-col gap-4"><label class="flex items-center gap-3 cursor-pointer p-4 rounded-xl border shadow-sm transition-all active:scale-95" style="background-color: var(--site-bg); border-color: hsl(var(--brand-hue), 80%, 95%);"><input type="checkbox" ${c.crd ? 'checked' : ''} onchange="uCake('crd', this.checked)" class="w-5 h-5 rounded"> <span class="font-bold text-sm" style="color: var(--site-text);">إرفاق بطاقة إهداء فاخرة.</span></label><label class="flex items-center gap-3 cursor-pointer p-4 rounded-xl border shadow-sm transition-all active:scale-95" style="background-color: var(--site-bg); border-color: hsl(var(--brand-hue), 80%, 95%);"><input type="checkbox" ${c.dlg ? 'checked' : ''} onchange="uCake('dlg', this.checked)" class="w-5 h-5 rounded"> <span class="font-bold text-sm" style="color: var(--site-text);">تفويض الإدارة لابتكار التصميم.</span></label></div></div></div></div><div class="p-8 md:p-14 border-t-2 flex flex-col md:flex-row justify-between items-center gap-8" style="background-color: hsl(var(--brand-hue), 80%, 95%); border-color: hsl(var(--brand-hue), 80%, 85%);"><div class="text-center md:text-right"><span class="block font-bold mb-2 text-lg md:text-xl opacity-70 uppercase tracking-widest" style="color: hsl(var(--brand-hue), 70%, 50%);">الإجمالي التقديري</span><div class="flex items-center gap-4 justify-center md:justify-start"><span class="text-4xl md:text-6xl font-bold drop-shadow-md" style="color: hsl(var(--brand-hue), 70%, 30%);">${price} ج.م</span><i data-lucide="award" class="w-10 h-10 animate-pulse hidden md:block" style="color: hsl(var(--brand-hue), 70%, 60%);"></i></div></div><button onclick="commitCakeBuilder()" class="w-full md:w-auto text-white font-bold text-xl md:text-2xl py-5 px-12 rounded-2xl shadow-xl hover:scale-105 active:scale-95 transition-all border-b-4 brand-gradient" style="border-color: hsl(var(--brand-hue), 70%, 40%);">إضافة للمراجعة</button></div></div>`;
    if(window.lucide) lucide.createIcons();
}
...
function renderCartCrossSell() {
    ...
    return `<div class="mt-8 animate-fade-in border-t border-dashed border-pink-200 pt-6"><p class="text-sm font-black text-gray-800 mb-4 flex items-center gap-2"><i data-lucide="sparkles" class="w-4 h-4 text-pink-500"></i> كملي اللحظة الحلوة بمنتجات تليق بيكي</p><div class="flex gap-4 overflow-x-auto pb-6 hide-scrollbar snap-slider">${suggestions.map(p => { const img = (p.images && p.images.length > 0) ? p.images[0] : (p.img || getImgFallback(p.category)); return `<div class="shrink-0 w-[260px] snap-slide bg-white border border-gray-100 rounded-2xl p-4 shadow-sm flex flex-col hover:shadow-md hover:border-pink-300 transition-all group"><div class="relative w-full h-36 mb-4 rounded-xl overflow-hidden border border-gray-50 bg-gray-50"><img src="${img}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105">${p.badge ? `<span class="absolute top-2 right-2 bg-pink-500 text-white text-[10px] px-2 py-1 rounded-lg font-bold shadow-md">${p.badge}</span>` : ''}</div><div class="flex-1 flex flex-col"><span class="text-[10px] font-bold text-pink-500 mb-1 tracking-wider bg-pink-50 self-start px-2 py-0.5 rounded-md">${escapeHTML(p.category)}</span><h5 class="text-[14px] font-bold text-gray-800 mb-1 leading-tight">${escapeHTML(p.name)}</h5><p class="text-[11px] text-gray-500 line-clamp-2 mb-4 font-bold opacity-90 leading-relaxed">${escapeHTML(p.desc || 'لمسة ساحرة من إبداعات حلويات بوسي تذوب في الفم.')}</p><div class="flex items-center justify-between mt-auto"><span class="text-[14px] text-pink-600 font-black">${p.price > 0 ? p.price + ' ج.م' : 'حسب الطلب'}</span><button onclick="addJS('${p.id}')" class="px-4 py-2 bg-white border border-pink-200 text-pink-500 rounded-xl flex items-center gap-1.5 hover:bg-pink-500 hover:text-white transition-all shadow-sm text-[11px] font-bold active:scale-95"><i data-lucide="plus" class="w-3.5 h-3.5"></i> إضافة</button></div></div></div>`; }).join('')}</div></div>`;
}
...
function renderCartList() {
    ...
    container.innerHTML = state.cart.map(item => {
        ...
        return `<div class="group flex items-center gap-3 p-3 bg-white border border-gray-100 rounded-2xl mb-3 hover:border-pink-200 transition-all shadow-sm"><div class="w-16 h-16 bg-pink-50 rounded-xl flex-shrink-0 flex items-center justify-center overflow-hidden border border-gray-50"><img src="${renderImg}" class="w-full h-full object-cover"></div><div class="flex-1 min-w-0 text-right"><h4 class="font-bold text-gray-800 text-[13px] line-clamp-1">${escapeHTML(item.name)}</h4><p class="text-[11px] text-pink-500 font-bold mt-1">${p} ج.م</p></div><div class="flex flex-col items-end gap-2"><button onclick="modQ('${identifier}', -${q})" class="p-1 text-gray-300 hover:text-red-500 transition-colors"><i data-lucide="trash-2" class="w-4 h-4"></i></button><div class="flex items-center gap-2 bg-gray-50 rounded-lg p-1 border"><button onclick="modQ('${identifier}', -1)" class="w-5 h-5 flex items-center justify-center rounded text-pink-500 hover:bg-pink-200 transition-all"><i data-lucide="minus" class="w-3 h-3"></i></button><span class="text-xs font-bold w-4 text-center">${q}</span><button onclick="modQ('${identifier}', 1)" class="w-5 h-5 flex items-center justify-center rounded text-pink-500 hover:bg-pink-200 transition-all"><i data-lucide="plus" class="w-3 h-3"></i></button></div></div></div>`;
    }).join('');
...
function addJS(id) {
    const safeId = String(id); const prod = catalogMap.get(safeId); if (!prod) return;
...
function calculateCartTotal() {
    ...
    const stText = document.getElementById('cart-subtotal-text'); if(stText) stText.innerText = sub + ' ج.م';
    const spText = document.getElementById('cart-shipping-text'); if(spText) spText.innerText = (shipFee > 0 ? '+' + shipFee : '0') + ' ج.م';
    const ctText = document.getElementById('cart-total-text'); if(ctText) ctText.innerText = (sub + shipFee) + ' ج.م';
}
    ...
    try {
        ...
        if (savedData) {
            ...
            if (data.method) {
                ...
                if (methodRadio) { methodRadio.checked = true; toggleDeliveryMethod(); }
            }
...
function backToCart() {
    const step1 = document.getElementById('step-1-cart'); const step2 = document.getElementById('step-2-checkout');
    if(step2) step2.classList.add('hidden'); if(step1) step1.classList.remove('hidden');
}
...
async function submitOrder() {
    ...
    try { localStorage.setItem('boseSweets_customer_data', JSON.stringify({ name: cName, phone: cPhone, method: deliveryMethod, area: cAreaId })); } catch(e) {}
...
function openProductDetails(id) {
    ...
    if(!modal) return;
    document.getElementById('pd-modal-title').innerText = prod.category;
    
    const imageList = (prod.images && prod.images.length > 0) ? prod.images : [prod.img || getImgFallback(prod.category)];
    
    const faqHtml = `
        <div class="mt-8 space-y-3 px-2">
            <h3 class="font-black text-lg text-gray-800 mb-4 flex items-center gap-2"><i data-lucide="help-circle" class="w-6 h-6 text-pink-500"></i> الأسئلة الشائعة</h3>
            <details class="details-faq-item bg-white p-5 rounded-2xl border shadow-sm group cursor-pointer transition-all hover:border-pink-200">
                <summary class="font-bold outline-none flex justify-between items-center text-gray-700">هل تحتوي منتجاتكم على مواد حافظة؟ <i data-lucide="chevron-down" class="w-5 h-5 text-gray-400 transition-transform group-open:rotate-180"></i></summary>
                <p class="text-sm text-gray-600 mt-4 leading-relaxed font-bold border-t pt-3 border-gray-100">جميع منتجات حلويات بوسي تُصنع يومياً بشغف من مكونات طبيعية 100% ولا نستخدم أي مواد حافظة لضمان تقديم أعلى معايير الجودة اللي تليق بيكم.</p>
            </details>
            <details class="details-faq-item bg-white p-5 rounded-2xl border shadow-sm group cursor-pointer transition-all hover:border-pink-200">
                <summary class="font-bold outline-none flex justify-between items-center text-gray-700">ازاي أقدر أحفظ المنتج عشان يفضل فريش؟ <i data-lucide="chevron-down" class="w-5 h-5 text-gray-400 transition-transform group-open:rotate-180"></i></summary>
                <p class="text-sm text-gray-600 mt-4 leading-relaxed font-bold border-t pt-3 border-gray-100">يُنصح دائماً بحفظ الحلويات في المبرد (الثلاجة) في علبتها المحكمة، ويفضل تقديمها بعد خروجها من المبرد بـ 10 دقائق لتستمتعوا بأفضل قوام ومذاق.</p>
            </details>
        </div>
    `;

    const related = catalog.filter(p => p.category === prod.category && String(p.id) !== String(prod.id)).sort(() => 0.5 - Math.random()).slice(0, 4);
    let relatedHtml = '';
    if(related.length > 0) {
        relatedHtml = `
            <div class="mt-10 px-2">
                <h3 class="font-black text-lg text-gray-800 mb-5 flex items-center gap-2"><i data-lucide="sparkles" class="w-6 h-6 text-pink-500"></i> قد يعجبك أيضاً</h3>
                <div class="flex gap-4 overflow-x-auto hide-scrollbar pb-4 snap-slider -mx-4 px-4">
                    ${related.map(r => {
                        const rImg = (r.images && r.images.length > 0) ? r.images[0] : (r.img || getImgFallback(r.category));
                        return `
                        <div class="shrink-0 w-40 snap-slide bg-white rounded-2xl shadow-sm border p-3 flex flex-col hover:shadow-md transition-all cursor-pointer group" onclick="openProductDetails('${r.id}')">
                            <div class="w-full h-28 rounded-xl overflow-hidden mb-3 border border-gray-50 bg-gray-50">
                                <img src="${rImg}" class="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500">
                            </div>
                            <h4 class="font-bold text-xs text-gray-800 line-clamp-2 mb-2">${escapeHTML(r.name)}</h4>
                            <span class="text-pink-600 font-black text-sm mt-auto">${r.price > 0 ? r.price + ' ج.م' : 'حسب الطلب'}</span>
                        </div>`
                    }).join('')}
                </div>
            </div>
    ...
    document.getElementById('pd-modal-content').innerHTML = `
        <div class="w-full aspect-square bg-gray-100 relative overflow-hidden snap-slider flex hide-scrollbar">
            ${imageList.map(url => `<img src="${url}" class="w-full h-full object-cover shrink-0 snap-slide">`).join('')}
            ${imageList.length > 1 ? `<div class="absolute bottom-6 w-full text-center z-10"><span class="bg-black/50 text-white text-[10px] px-3 py-1.5 rounded-full backdrop-blur-md font-bold border border-white/20 shadow-lg">اسحب لمشاهدة باقي الصور (${imageList.length})</span></div>` : ''}
        </div>
        <div class="p-6 bg-gray-50 -mt-6 relative z-10 rounded-t-[2rem]">
            <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 mb-6 relative overflow-hidden">
                <div class="absolute top-0 right-0 w-16 h-16 bg-pink-50 rounded-bl-[100px] -z-10"></div>
                <h2 class="text-2xl font-black text-gray-900 leading-tight mb-2">${escapeHTML(prod.name)}</h2>
                ${prod.badge ? `<span class="inline-block bg-pink-100 text-pink-600 text-[10px] font-black px-2 py-1 rounded-md mb-3">${prod.badge}</span>` : ''}
                <div class="mt-4 pt-4 border-t border-dashed border-gray-200">
                    <h3 class="font-black text-sm text-gray-400 mb-2">وصف المنتج</h3>
                    <p class="text-sm font-bold text-gray-700 leading-loose">${escapeHTML(prod.desc || 'لمسة ساحرة من إبداعات حلويات بوسي تذوب في الفم، مصنوعة بحب وعناية فائقة لتليق بمناسباتكم.')}</p>
                </div>
            </div>
            ${faqHtml}
            ${relatedHtml}
    ...
    if (isOutOfStock) {
        addBtn.className = "w-full text-gray-500 font-black text-xl py-4 md:py-5 rounded-2xl flex items-center justify-center gap-3 bg-gray-200 cursor-not-allowed";
        addBtn.innerHTML = `<i data-lucide="clock" class="w-6 h-6"></i> <span>نفدت الكمية مؤقتاً</span>`;
        addBtn.onclick = null;
    } else {
        addBtn.className = "w-full text-white font-black text-xl py-4 md:py-5 rounded-2xl shadow-xl flex items-center justify-center gap-3 brand-gradient active:scale-95 transition-all";
        addBtn.innerHTML = `<i data-lucide="shopping-cart" class="w-6 h-6"></i> <span>إضافة للسلة</span> <span class="opacity-50">|</span> <span class="drop-shadow-md">${prod.price > 0 ? prod.price + ' ج.م' : 'حسب الطلب'}</span>`;
        addBtn.onclick = () => { addJS(prod.id); closeProductDetails(); };
    }

    modal.classList.remove('hidden'); modal.classList.add('flex');
    if(window.lucide) lucide.createIcons();
    setTimeout(() => { modal.classList.remove('opacity-0'); document.getElementById('pd-modal-content').classList.add('details-slide-up'); }, 10);
}

function closeProductDetails() {
    const modal = document.getElementById('product-details-modal');
    modal.classList.add('opacity-0');
    document.getElementById('pd-modal-content').classList.remove('details-slide-up');
...
function showInfo(t) {
    ...
    const m = document.getElementById('info-modal'); if(m) { m.classList.remove('hidden'); m.classList.add('flex'); if(window.lucide) lucide.createIcons(); }
}
