...
// 👑 مصمم التورتات المتعدد الخطوات (المكتمل سيادياً)
...
export const renderCartList = function() {
    ...
    container.innerHTML = state.cart.map(item => {
        ...
        <div class="cart-item-premium flex flex-col bg-[#ffffff] p-4 rounded-[2rem] border-2 border-[var(--brand-primary)] shadow-sm relative mb-4">
            <div class="absolute top-4 left-4 z-10"><button onclick="window.modQ('${identifier}', 'remove')" class="p-2 text-[var(--brand-primary)] hover:bg-[var(--brand-primary)] hover:text-[#ffffff] border-2 border-transparent hover:border-[var(--brand-primary)] rounded-xl transition-all"><i data-lucide="trash-2" class="w-5 h-5"></i></button></div>
            <div class="flex items-start gap-4 mb-4">
                <div class="w-20 h-20 md:w-24 md:h-24 rounded-[1.5rem] overflow-hidden shrink-0 bg-[#ffffff] border-2 border-[var(--brand-primary)] p-1 flex items-center justify-center"><img src="${renderImg}" class="w-full h-full object-contain" onerror="this.onerror=null; this.src=window.getImgFallback('${escapeHTML(item.category)}');"></div>
...
export const renderSmartSuggestions = function(context = 'main') {
    ...
    const sortedProducts = availableProducts.sort((a, b) => { return (userPrefs[b.category] || 0) - (userPrefs[a.category] || 0) + (0.5 - Math.random()); });
