...
(function () {
    ...
    async function loadBoseStoreDataFromSupabase() {
        ...
        return {
            ...
            // 🐛🛡️ [نفس فخ homepage/promotions/loyalty بالحرف - about.html صفحة "من نحن"]:
            // عمود about موجود فعليًا في store_settings ومليان بمحتوى حقيقي (قصة
            // المتجر كاملة)، وبيتقرا من الـ select=* هنا، لكنه كان الحقل الوحيد
            // المتبقي اللي منسي من الـ object الراجع تحت. about.html كان بيشوف
            // window.BoseStoreData.about == undefined دايمًا، فبيرجع بدري من غير
            // ما يعرض ولا حرف من قسم القصة/الإحصائيات/القيم (Object.keys({}).length
    ...
    async function submitBoseReview({ productId, userName, rating, comment, images }) {
        // 🐛→✅ [إصلاح جذري - مراجعات بتتفشل بصمت]: كانت هنا "return=representation"،
        // ومعناها إننا بنطلب من Supabase يرجّعلنا الصف اللي اتسجل بعد الإدراج مباشرة.
        // المشكلة إن سياسة القراءة العامة لجدول reviews بتسمح تشوف بس المراجعات
        // المعتمدة (is_approved = true)، والمراجعة الجديدة دايماً بتتسجل is_approved
        // = false (قيد المراجعة) - يعني طلب إرجاع الصف ده كان بيتصادم مع سياسة
        // القراءة، وبوستجرس كان بيرفض الإدراج بالكامل برسالة "new row violates row-
        // level security policy" حتى إن شرط الإدراج نفسه كان سليم 100%. النتيجة:
        // كل مراجعة كانت بتتفشل فعلياً في قاعدة البيانات من غير أي خطأ واضح للعميلة
        // ولا لينا - كانت بس بتفضل عالقة على "بيتم الإرسال..." للأبد. تأكدت من
        // السبب ده مباشرة عن طريق تجربة نفس الإدراج حرفياً على القاعدة الحقيقية.
        // الحل: "return=minimal" - إحنا مش محتاجين الصف يرجعلنا أصلاً (الكود عندنا
        // مش بيستخدم القيمة الراجعة في أي مكان)، فمفيش داعي نمر على سياسة القراءة خالص.
        // 🐛🛡️ [إصلاح]: باقي استخدامات boseSupabaseFetch كلها قراءة (GET) آمنة
        // للمحاولة التلقائية، لكن الطلب ده POST بيضيف مراجعة فعلية في القاعدة -
        // لو الشبكة اتقطعت بعد ما الإدراج نجح فعليًا على السيرفر بس الرد ضاع في
        // الطريق (حالة شائعة على نت موبايل ضعيف)، كان الكود بيعيد نفس POST تاني
        // (وممكن تالتة كمان) فيسجل نفس المراجعة 2-3 مرات في القاعدة من غير ما
        // العميلة تعرف. بنمرر retries=0 صراحة عشان العملية دي متتكررش تلقائيًا -
        // لو فشلت فعلاً، العميلة هتشوف رسالة خطأ واضحة وتقدر تضغط "إرسال" تاني
        // بوعي منها، بدل تكرار صامت من غير علمها.
    ...
    async function saveBoseOrderToDatabase(completedBoseOrderObject) {
        ...
        // 🌸 [نظام التعرّف على العميل]: عمود الملاحظات في قاعدة البيانات واحد
        // بس (notes)، فمفيش عمود منفصل لملاحظات الشحن. عشان ملاحظة الشحن
