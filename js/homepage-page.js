...
(function () {
    ...
    async function init() {
        ...
        if (homepageData.rotation?.lastRunAt) {
            const lastRunDate = new Date(homepageData.rotation.lastRunAt);
            lastRunEl.textContent = `آخر تحديث تلقائي: ${lastRunDate.toLocaleString("ar-EG", { dateStyle: "medium", timeStyle: "short" })}`;
        } else {
            lastRunEl.textContent = "لسه مفيش تحديث تلقائي اتنفّذ.";
        }
