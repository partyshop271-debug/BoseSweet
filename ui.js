...
window.loadLiveReviews = async function(productId) {
    ...
    try {
        ...
        if (snapshot.empty) {
            reviewsContainer.innerHTML = '<p class="text-xs text-slate-400 font-bold text-center py-4">كن أول من يشارك تجربته مع الصنف ده... </p>';
            return;
        }
            ...
            <div class="space-y-4 text-right flex flex-col h-full justify-between">
                ...
                <div class="pt-4 border-t border-pink-50 space-y-4">
                    <div class="flex justify-between items-center bg-pink-50/50 p-4 rounded-xl">
                        <span class="text-xs font-bold text-slate-500">السعر:</span>
                    ...
                    <div class="flex gap-2">
                        ...
                        ${isOutOfStock ? 
                            ...
                            `<button onclick="window.addWithQtyContext(this, '${prod.id}')" class="flex-1 py-3 btn-premium-action brand-gradient text-white rounded-full font-black text-xs text-center shadow-md">إضافة للسلة 🛍️</button>`
                        }
                    </div>
                </div>
                <div class="pt-6 mt-4 border-t border-pink-50 w-full">
                    ...
                    <div class="mt-6 bg-slate-50 p-4 rounded-[2rem] border border-pink-100">
                        <h4 class="font-black text-slate-800 text-xs mb-3 flex items-center gap-1.5"><i data-lucide="edit-3" class="w-4 h-4 text-[#ff3377]"></i> رأيك بيفرق معانا.. شاركنا تجربتك مع طعم حلويات بوسي</h4>
                        <div class="space-y-3">
                            <input type="text" id="review-cust-name-${prod.id}" placeholder="اسمك الكريم..." class="w-full p-3 bg-white border border-pink-100 rounded-xl text-xs font-bold focus:outline-none focus:border-[#ff3377]">
                            <textarea id="review-cust-comment-${prod.id}" rows="2" placeholder="احكيلنا عن تجربتك، الطعم، الجودة..." class="w-full p-3 bg-white border border-pink-100 rounded-xl text-xs font-bold focus:outline-none focus:border-[#ff3377] resize-none"></textarea>
                            ...
                            <button id="review-submit-btn-${prod.id}" onclick="window.submitCustomerReviewLive('${prod.id}')" class="w-full py-2.5 bg-[#ff3377] text-white rounded-xl text-xs font-black shadow-sm">إرسال التقييم</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    if (window.currentBuilderStep === 1) {
        stepContentHTML = `
            <div class="p-10 text-center bg-pink-50 border-b border-pink-100 relative z-10 rounded-t-[3rem]">
                <h2 class="text-3xl font-black mb-4 uppercase tracking-tight text-pink-600">صمم تورتة مناسبتك السعيدة 👑</h2>
                <p class="text-base font-bold text-slate-600 max-w-2xl mx-auto">اختار أدق التفاصيل لتصميم تورتتك، وسيب الباقي علينا.</p>
            </div>
            <div class="cake-builder-step-panel step-active p-8 md:p-12 space-y-8 bg-white">
                <div class="space-y-4">
                    <label class="block font-black text-lg text-slate-800 flex items-center gap-3"><i data-lucide="cake" class="w-5 h-5 text-pink-600"></i> نكهة الكيك الأساسي</label>
        ...
        stepContentHTML = `
            ...
            <div class="cake-builder-step-panel step-active p-8 md:p-12 space-y-8 bg-white">
                <div class="space-y-4">
                    <label class="block font-black text-lg text-slate-800 flex items-center gap-3"><i data-lucide="users" class="w-5 h-5 text-pink-600"></i> عدد الأفراد</label>
                    ...
                    <p class="text-sm text-slate-500 text-center font-bold">حساب التسعير: المتر للفرد يعادل ${basePrice} ج.م</p>
                </div>
                <div class="space-y-4 pt-4 border-t border-slate-50">
                    <label class="block font-black text-lg text-slate-800 flex items-center gap-3"><i data-lucide="printer" class="w-5 h-5 text-pink-600"></i> دمج وطباعة الصور</label>
                ...
                <div class="flex flex-col sm:flex-row justify-between gap-4 pt-6 mt-4">
                    <button onclick="window.changeBuilderStep(-1)" class="px-8 py-4 bg-slate-100 text-slate-600 font-black text-sm rounded-full active:scale-95 border border-slate-200">➡️ السابق</button>
                    <button onclick="window.changeBuilderStep(1)" class="px-8 py-4 btn-premium-action brand-gradient text-white font-black text-sm rounded-full shadow-lg">التالي: الملاحظات والتأكيد ⬅️</button>
                </div>
            </div>`;
    }
        ...
        stepContentHTML = `
            <div class="p-10 text-center bg-pink-50 border-b border-pink-100 relative z-10 rounded-t-[3rem]">
                <h2 class="text-3xl font-black mb-4 uppercase tracking-tight text-pink-600">مراجعة وتأكيد الطلب</h2>
            </div>
            <div class="cake-builder-step-panel step-active p-8 md:p-12 space-y-8 bg-white">
                <div class="space-y-4">
                    <label class="block font-black text-lg text-slate-800 flex items-center gap-3"><i data-lucide="edit-3" class="w-5 h-5 text-pink-600"></i> ملاحظات للتنفيذ أو عبارات للطباعة</label>
                    <textarea id="builder-notes-textarea" rows="4" oninput="cakeState.notes = this.value" class="w-full p-5 bg-slate-50 border-2 border-slate-100 rounded-[2rem] font-bold text-base focus:outline-none focus:border-pink-500 resize-none" placeholder="اكتب الاسم، السن، الألوان المفضلة، أو أي تفاصيل تصميم تود إبلاغنا بيها...">${escapeHTML(cakeState.notes)}</textarea>
                </div>
                
                <div class="bg-pink-50 p-6 rounded-[2rem] border border-pink-100 space-y-3">
                    <h4 class="font-black text-slate-800 text-lg mb-4 border-b border-pink-200 pb-3 flex items-center gap-2"><i data-lucide="file-text" class="w-5 h-5 text-pink-600"></i> ملخص تصميم التورتة:</h4>
                    <div class="flex justify-between items-center text-sm"><span class="font-bold text-slate-500">النكهة:</span><span class="font-black text-slate-800">${cakeState.flavor}</span></div>
                    <div class="flex justify-between items-center text-sm"><span class="font-bold text-slate-500">الشكل:</span><span class="font-black text-slate-800">${cakeState.shape}</span></div>
                    <div class="flex justify-between items-center text-sm"><span class="font-bold text-slate-500">تكفي:</span><span class="font-black text-slate-800 font-mono">${cakeState.persons} أفراد</span></div>
                    <div class="flex justify-between items-center text-sm"><span class="font-bold text-slate-500">إضافة صورة:</span><span class="font-black text-slate-800">${cakeState.printing}</span></div>
                    <div class="flex justify-between items-center pt-4 border-t border-pink-200 mt-4 text-lg font-black">
                        <span class="text-slate-800">القيمة التقديرية:</span>
                ...
                <div class="flex flex-col sm:flex-row justify-between gap-4 pt-6 mt-4">
                    ...
                    <button onclick="window.commitCakeBuilderToCart()" class="px-8 py-4 btn-premium-action brand-gradient text-white font-black text-lg rounded-full shadow-xl flex-1 text-center">تأكيد وإضافة للسلة 👑</button>
                </div>
            </div>`;
    }
    ...
    container.innerHTML = state.cart.map(item => {
        ...
        <div class="cart-item-spacious flex items-center gap-6 bg-white p-4 rounded-[2rem] border border-slate-100 shadow-sm">
            ...
            <div class="flex-1 text-right min-w-0">
                ...
                <p class="text-xs font-bold text-slate-500 mb-2 truncate">${escapeHTML(item.desc || 'منتج مميز بطلبك.')}</p>
...
window.showInfo = function(t) {
    const d = { about: { t: 'عن حلويات بوسي', b: siteSettings.footerQuote || 'العلامة التجارية الرائدة في صناعة الحلويات الفاخرة بالفرافرة.' }, privacy: { t: 'سياسة الأمان والبيانات', b: 'نلتزم في حلويات بوسي بحماية بيانات عملائنا وفق أعلى معايير الخصوصية.' }, refund: { t: 'سياسة الاستبدال والاسترجاع', b: 'تخضع كافة الطلبات لمعايير رقابة الجودة عشان نضمن رضاك التام.' } };
    if(!d[t]) return;
...
window.submitCustomerReviewLive = async function(productId) {
    ...
    if (!customerName || !comment) {
        showSystemToast("عفواً، يرجى كتابة الاسم ورأيك لإرسال التقييم ✨", "error");
        return;
    }
