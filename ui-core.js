/**
 * 👑 BoseSweets UI Core Engine (V24.5 - Sovereign Monitor Edition)
 * المحرك الأساسي والوظائف السيادية - حلويات بوسي
 * تم الدمج والترقية: تحصين دوال رسم الشريط العلوي وتطبيق الإعدادات الهيكلية.
 * 🛡️ التحديث الأمني: زراعة مستشعر BoseMonitor لمراقبة استقرار النواة البصرية وتدفق البيانات.
 */
...
/**
 * 👑 تأمين الصور الافتراضية (Sovereign Image Fallback)
 */
export const getImgFallback = function(category) {
    try {
        ...
        if(window.BoseMonitor) window.BoseMonitor.report(error, 'ui-core.js', null, null, 'getImgFallback');
        return 'https://res.cloudinary.com/dyx4w0dr1/image/upload/v1712586716/logo_bose_gold.jpg';
    }
};
window.getImgFallback = getImgFallback;

/**
 * 👑 محرك توليد الأوصاف الاحترافي (Professional Description Engine)
 * يعتمد على تحليل اسم الصنف والقسم لتقديم وصف يليق بفخامة حلويات بوسي.
 */
export const getFinalDescription = function(p) {
    try {
        ...
        if(window.BoseMonitor) window.BoseMonitor.report(error, 'ui-core.js', null, null, 'getFinalDescription');
        return 'منتج فاخر من حلويات بوسي.';
    }
};
...
/**
 * 👑 شريط الإعلانات السيادي (Sovereign Ticker Engine)
 * تم التحديث لضمان التموضع الصحيح وحماية الهيدر من التداخل.
 */
export const renderTicker = function() {
    try {
        ...
        if(window.BoseMonitor) window.BoseMonitor.report(e, 'ui-core.js', null, null, 'renderTicker');
        console.warn("BoseSweets: تم تجاوز خطأ في الشريط المتحرك لضمان سير الواجهة.");
    }
};
window.renderTicker = renderTicker;

/**
 * 👑 تطبيق إعدادات الموقع والهوية البصرية (Visual Identity Enforcement)
 */
export const applySettingsToUI = function() {
    try {
        ...
        if (loaderTextEl) {
            loaderTextEl.innerText = (settings.UI_Settings && settings.UI_Settings.loader_text) 
                ? settings.UI_Settings.loader_text 
                : ((settings.visuals && settings.visuals.loaderText) ? settings.visuals.loaderText : "حلويات بوسي ✨");
        }

        // تحصين الـ SEO والبيانات الميتا
        if (settings.seo) {
            document.title = (settings.seo.title && settings.seo.title.trim() !== '') ? settings.seo.title.trim() : `${settings.brandName || 'حلويات بوسي'} | المنصة الرسمية المعتمدة`;
            ...
            metaDesc.setAttribute('content', settings.seo.desc || `الموقع الرسمي لبراند حلويات بوسي (BoseSweets). نتميز بصناعة التورت الملكية والحلويات الفاخرة.`);
            ...
            metaKeywords.setAttribute('content', settings.seo.keywords || `حلويات بوسي, BoseSweets, تورت الفرافرة, حلويات الوادي الجديد`);
        }

        // تطبيق أنماط اللودر العالمي
        ...
        if(areaSelect && shippingZones) {
            areaSelect.innerHTML = `<option value="" disabled selected>اختار منطقة التوصيل...</option>` + 
                shippingZones.map(z => `<option value="${z.id}">${escapeHTML(z.name)} (+${Number(z.fee)} ج.م توصيل)</option>`).join('');
        }
        ...
        if(window.BoseMonitor) window.BoseMonitor.report(e, 'ui-core.js', null, null, 'applySettingsToUI');
...
/**
 * 👑 رسم بطاقة المنتج الموحدة (Sovereign Product Card Builder)
 */
export const drawProductCard = function(p) {
    try {
        ...
        if(window.BoseMonitor) window.BoseMonitor.report(error, 'ui-core.js', null, null, 'drawProductCard');
        return '<div class="p-4 border-2 border-red-200 rounded-xl">خطأ في عرض المنتج.</div>';
    }
};
window.drawProductCard = drawProductCard;

/**
 * 👑 إدارة المراجعات المباشرة (Sovereign Live Reviews)
 */
export const submitCustomerReviewLive = async function(productId) {
    try {
        ...
        if(window.BoseMonitor) window.BoseMonitor.report(e, 'ui-core.js', null, null, 'submitCustomerReviewLive');
...
export const loadLiveReviews = async function(productId) {
    try {
        ...
        if(window.BoseMonitor) window.BoseMonitor.report(error, 'ui-core.js', null, null, 'loadLiveReviews');
        const reviewsContainer = document.getElementById(`reviews-list-${productId}`);
        if(reviewsContainer) reviewsContainer.innerHTML = '<p class="text-xs text-[var(--site-text)] font-bold text-center col-span-full py-4">جاري مزامنة الآراء...</p>';
    }
};
...
if (typeof window !== 'undefined') {
    try {
        ...
        console.log("👑 BoseSweets Engine: UI Core finalized with Sovereign Monitoring.");
    } catch (error) {
        if(window.BoseMonitor) window.BoseMonitor.report(error, 'ui-core.js', null, null, 'Final Window Bindings Failure');
    }
}