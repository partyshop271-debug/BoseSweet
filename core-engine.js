...
(function () {
    ...
    function ensureSharedLayoutHubs(storeData) {
        ...
        // 🚨 [إصلاح فيديو المشكلة 1]: حقن التيكر العلوي كأول عنصر هندسي مقدّس في الـ body قبل الهيدر حتماً ليكون فوقه
        let tickerNode = document.querySelector('.bose-ticker-section');
        if (!tickerNode) {
        ...
        if (tickerNode.innerHTML.trim() === "" && storeData.navigation && storeData.navigation.topBarMessages) {
            const messagesHTML = storeData.navigation.topBarMessages.map(msg => `
                <div class="bose-ticker-item"><i class="fas fa-crown"></i> ${msg}</div>
            `).join('');
        ...
        if (!headerNode) {
            ...
            // إدخال الهيدر مباشرة بعد وسم التيكر لضمان الترتيب التأسيسي الصارم للموقع
    ...
    /* 🛡️ [إصلاح فيديو المشكلة 5]: تزويد السلايدر بأسهم ونقاط تفاعلية وتجربة سحب ممتدة تمنع القفز */
    function renderBoseCategoriesSlider(storeData) {
        ...
        if (!controlsContainer && wrapper) {
            ...
            // حقن أسهم التنقل الفاخرة المدمجة بالنقاط بحسب طلبك في فيديو المشاكل لقسم تسوق حسب الفئة
            ...
            let currentCatInx = 0;
            const cardWidth = 304; // 280 عرض + 24 فجوة ممتدة هندسياً بدقة متناهية بالمسطرة
    ...
    /* 🧠 [إصلاح فيديو المشكلة 3]: حل جذري ومطور لثغرة الكاروسيل وقابلية السحب واللمس الناعمة دون أي انقطاع لقسم عقد من الإتقان */
    ...
    /* ==========================================================================\
       3. نظام الإشعارات الفوري المنبثق هندسياً في منتصف الشاشة لمواجهة عين العميل
       ========================================================================== */
    ...
    window.attachProductCardEvents = function (containerElement, productsList) {
    ...
    function initializeGlobalFeatures() {
        ...
        /* 👑 [إصلاح فيديو المشكلة 4]: هندسة التخصيص الحصرية: عزل كامل لطبقة الخلفية البمبي عن طبقة صور المنتجات الحقيقية لمنع التوحد */
